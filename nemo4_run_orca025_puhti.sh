...
run_length=$(( 365*24*60*60/$rn_rdt ))
# how many cycles to run [10 years, does not change]
ncycles=2

# how many cycles have already been completed? 
# Indicated by how many ocean.output.${ncycle} files we have
ncycle=2
for n in `seq 1 $ncycles`
do
...
if [ -f namelist_ice_cfg.orig ]; then 
...
then
    ...
    echo "NEMO restart files not found after run."
fi
